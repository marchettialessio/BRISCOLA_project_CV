...
static std::vector<cv::Point2f> orderPoints(const std::vector<cv::Point>& polygon) {
	...
	for (int i = 1; i < 4; ++i) {
		...
		if (sum < minSum) {          // somma minima trovata -> nuovo candidato top-left
			minSum = sum;
			topLeft = i;
		}
		if (sum > maxSum) {          // somma massima trovata -> nuovo candidato bottom-right
			maxSum = sum;
			bottomRight = i;
		}
		if (diff < minDiff) {        // differenza minima -> nuovo candidato top-right
			minDiff = diff;
			topRight = i;
		}
		if (diff > maxDiff) {        // differenza massima -> nuovo candidato bottom-left
			maxDiff = diff;
			bottomLeft = i;
		}
	}
...
int main(int argc, char** argv) {
	...
	const std::string imagePath = argc >= 2
		? argv[1]
		: "D:/UNI/LabCV/BRISCOLA/BRISCOLA_project_CV/test image/1.png";
